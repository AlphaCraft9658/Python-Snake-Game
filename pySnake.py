    ...
    def check_for_self_collision(self):
        ...
        for index, tile in enumerate(self.body):
            if index < len(self.body) - 1:
                ...
                if len(self.key_pattern) > 0:
                    if self.self_collide_check_cursor == [(self.head[0] + self.directions[self.key_pattern[0]][0]) % 25,
                                                          (self.head[1] + self.directions[self.key_pattern[0]][1]) % 25]:
                        change_score_data(self, False)
                        return True
                else:
                    if self.self_collide_check_cursor == [(self.head[0] + self.directions[self.direction][0]) % 25,
                                                          (self.head[1] + self.directions[self.direction][1]) % 25]:
                        change_score_data(self, False)
                        return True
        return False
