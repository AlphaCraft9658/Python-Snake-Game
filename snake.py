...
for i in range(10):
    for n in range(10):
        pygame.draw.rect(display_grid, (0, 175, 255), (n % 2 * 50 + i * 50 + i * 50, n * 50, 50, 50))
...
class Snake:
    ...
    def move(self):
        ...
        if time() - self.saved_time > self.tbs:
            ...
            if not game_lost:
                ...
                if not screen_rect.contains(Rect((self.head[0] + self.directions[self.direction][0]) * 50,
                                                 (self.head[1] + self.directions[self.direction][1]) * 50, 50, 50)):
                    started = False
                    game_lost = True
                    fade_direction = 0
                    start_text.set_alpha(0)
                ...
                if self.length < 2:
                    if len(self.body) > 0:
                        self.body.append((4 - (self.head[0] + self.body[-1][0]), 4 - (self.head[1] + self.body[-1][1])))
                    else:
                        self.body.append(
                            (self.directions[self.direction][0] * -1, self.directions[self.direction][1] * -1))
                    self.length += 1
    ...
    def draw(self, surface):
        pygame.draw.rect(surface, (255, 219, 5), (self.head[0] * 50, self.head[1] * 50, 50, 50))
        self.cursor = self.head.copy()
        for v in self.body:
            self.cursor[0] += v[0]
            self.cursor[1] += v[1]
            pygame.draw.rect(surface, (255, 219, 5), (self.cursor[0] * 50, self.cursor[1] * 50, 50, 50))
...
class Food:
    ...
    def randomize_position(self):
        while True:
            self.grid_pos = (randint(0, 9), randint(0, 9))
    ...
    def draw(self, surface):
        pygame.draw.rect(surface, (254, 1, 0), (self.grid_pos[0] * 50 + 10, self.grid_pos[1] * 50 + 10, 30, 30))
...
while run:
    ...
    if started:
        ...
        if not game_lost:
            ...
            draw_score()
            if snake.length > 1:
                snake.tbs = .25 - (snake.length - 2) * .005
                if snake.tbs < .125:
                    snake.tbs = .125
